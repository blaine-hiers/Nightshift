...
## Run report — write to docs/runs/YYYY-MM-DD-drain.md and commit
    ...
    |---|---|---|---|---|---|
    (Outcome: PR #N / needs-info / blocked / failed-honestly / parked / decomposed / out-of-scope / deferred-to-wave-N)
    ## Off-ramped this run   <- what a human has to unstick
    | Issue | Status | Waiting on | Owner |
    |---|---|---|---|
    (every ticket this run moved to Needs Input or Blocked; "none" if none)
    ...
    - Label hygiene: {tickets whose Repo or Tier this run had to derive —
      the `*` marks in the triage table} / {tickets fetched} (a rising
      number means tickets are reaching Todo untriaged)
    ...
    - a tree kept for a `Blocked` issue is expected, not a leak — label it so, and flag it if it holds credentials and the block is open-ended
    ## Gate-erosion check
